...
{
	public abstract class MaterialOld
	{
		...
		/// </summary>
		public virtual Float3 Emit(in TraceQuery query, ExtendedRandom random) => SampleTexture(EmissionMap, Emission, query.shading.texcoord);
		...
		/// </summary>
		public abstract Float3 BidirectionalScatter(in TraceQuery query, ExtendedRandom random, out Float3 direction);

		// public unsafe void FillTangentNormal(ref TraceQuery query)
		// {
		...
		protected bool AlphaTest(in TraceQuery query, out Float3 color)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		protected bool CullBackface(in TraceQuery query) => BackfaceCulling && query.ray.direction.Dot(query.normal) > 0f;
