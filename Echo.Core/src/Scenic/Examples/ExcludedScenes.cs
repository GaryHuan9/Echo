...
public class BallRoom : Scene
{
	public BallRoom()
	{
		Add(new AmbientLight { Texture = new Cubemap("Assets/Cubemaps/OutsideDayTime") { Tint = Tint.Scale(new RGBA128(0.02f)) } });
		...
		Add(new MeshEntity { Mesh = mesh, MaterialLibrary = materials });

		Add(new Camera(84f) { Position = new Float3(2.2f, 1.7f, -7.3f), Rotation = new Float3(-2f, 76f, 0f) });
	}
}