...
sealed class SegmentReader : IDisposable
{
	...
	{
		if (!SkipWhiteSpace()) return CharSpan.Empty;
	...
	public static bool EqualsSingle(CharSpan span, char target) => span.Length == 1 && span[0] == target;
...
static class TypeMap
{
	...
	public static Type Find(string identifier) => map.TryGetValue(identifier);
}