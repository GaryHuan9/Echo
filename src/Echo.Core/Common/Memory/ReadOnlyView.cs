...
public readonly struct ReadOnlyView<T>
{
	...
	/// </summary>
	public ref readonly T this[int index] => ref Unsafe.Add(ref array[0], EnsureShift(index));
	...
	public ReadOnlyView<T> Slice(int offset, int length) => new(array, EnsureShift(offset), length);
	...
	public ReadOnlySpan<T> AsSpan(int offset) => array.AsSpan(EnsureShift(offset), Length);

	/// <inheritdoc cref="AsSpan()"/>
	public ReadOnlySpan<T> AsSpan(int offset, int length) => array.AsSpan(EnsureShift(offset), length);
