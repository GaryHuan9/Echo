...
public class SceneUI : AreaUI
{
	...
	protected override void Update(in Moment moment)
	{
		...
		if (ImGuiCustom.BeginProperties("Info"))
		{
			...
			ImGuiCustom.Property("Enclosing Box", info.aabb.ToString(DefaultFormat.Floating));
			ImGuiCustom.Property("Enclosing Sphere", info.boundingSphere.ToString(DefaultFormat.Floating));
